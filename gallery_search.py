...
#        self.exhib_intro_lemm, _ = self.lemmatize_text_with_ner(self.exhib_intro) # the second output left blank
    ...
    def search(self, query):
        ...
        if query and not str(query).isspace():
            query = str(query).strip().replace("(", " ( ")      # remove starting & ending whitespaces
            #add spaces after/before brackets
            query = query.replace("(", " ( ").replace(")", " ) ")  #extra space will be removed later in self.remove_unknown_terms()
