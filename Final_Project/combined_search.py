...
# for lemmatization:
tv_lemm = TfidfVectorizer(lowercase=True, sublinear_tf=True, use_idf=True, norm="l2")
sparse_matrix_r_lemm = tv_lemm.fit_transform(exhib_articles_lemm).T.tocsr()        # sparse_matrix_r: 'r' for 'relevance'

cv_lemm = CountVectorizer(lowercase=True, binary=True)   
sparse_matrix_b_lemm = cv_lemm.fit_transform(exhib_articles_lemm).T.tocsr()        # sparse_matrix_b：'b' for 'boolean'

terms_lemm = cv_lemm.get_feature_names_out()
t2i_lemm = cv_lemm.vocabulary_ 
...
def relevance_search(query, is_wildcard):  
    ...
    # use lemmatized matrix otherwise
        ...
        query_vec = tv_lemm.transform([query.lower()]).tocsc()       #convert query to vector
        hits = np.dot(query_vec, sparse_matrix_r_lemm)
...
def search():
    ...
    if query:
        ...
        invalid_words = invalid_term(query_lemm, terms_lemm)
        ...
        if not invalid_words:  
            ...
            for q in query_list:
                ...
                # 2: relevance search    
                    ...
                    idx_matches_per_loop = relevance_search(q, is_wildcard)
